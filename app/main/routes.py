...
@bp.route('/useq_par_nx_par_over_par_one_plus_nx_par', methods=['GET', 'POST'])
def useq_par_nx_par_over_par_one_plus_nx_par():
    form = USeqParNXParOverParOnePlusNXParForm()

    if form.validate_on_submit():
        flash('k: {}, x_input: {}, a: 0.1'\
            .format(form.k.data, form.x_input.data))
        sequence = SequenceUniform()
        sequence.par_nx_par_over_par_one_plus_nx_par(form.k.data, \
            form.x_input.data)
        return redirect(url_for(\
            'main.useq_par_nx_par_over_par_one_plus_nx_par_graph_results'))
    return render_template(\
        'calculus/sequences_of_functions/uniform/'\
        'useq_par_nx_par_over_par_one_plus_nx_par.html', \
        title='Math Website', form=form)


@bp.route('/useq_par_nx_par_over_par_one_plus_nx_par_graph_results', \
        methods=['GET', 'POST'])
def useq_par_nx_par_over_par_one_plus_nx_par_graph_results():
    return render_template(\
        'calculus/sequences_of_functions/uniform/'\
        'useq_par_nx_par_over_par_one_plus_nx_par_graph_results.html', \
        title='Math Website')


@bp.route('/useq_par_nx_par_over_par_one_plus_par_nx_par_squared_par', \
        methods=['GET', 'POST'])
def useq_par_nx_par_over_par_one_plus_par_nx_par_squared_par():
    form = USeqParNXParOverParOnePlusParNXParSquaredParForm()
    ...
    if form.validate_on_submit():
        ...
        sequence = SequenceUniform()
        sequence.par_nx_par_over_par_one_plus_par_nx_par_squared_par( \
                form.k.data, form.x_input.data)
        return redirect(url_for(\
            'main.'\
            'useq_par_nx_par_over_par_one_plus_par_nx_par_squared_par_'\
            'graph_results'))
    return render_template(\
        'calculus/sequences_of_functions/uniform/'\
        'useq_par_nx_par_over_par_one_plus_par_nx_par_squared_par.html', \
        title='Math Website', form=form)


@bp.route('/useq_par_nx_par_over_par_one_plus_par_nx_par_squared_par_'\
        'graph_results', methods=['GET', 'POST'])
def useq_par_nx_par_over_par_one_plus_par_nx_par_squared_par_graph_results():
    return render_template(\
        'calculus/sequences_of_functions/uniform/'\
        'useq_par_nx_par_over_par_one_plus_par_nx_par_squared_par_'\
        'graph_results.html', \
...
def l_trominoes():
    return render_template(
            'l_trominoes/l_trominoes.html', title='Math Website')
